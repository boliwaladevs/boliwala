...
import { getLendersWithCounts, getDistrictsWithCounts, parseSearchFilters, searchListings, PAGE_SIZE, LENDER_TYPES, LENDER_TYPE_LABELS, type SearchParamsInput } from "@/lib/data/listings"
...
export async function PropertyResults({ searchParams }: { searchParams: SearchParamsInput }) {
  ...
  const [{ listings, totalCount }, lenders, districts] = await Promise.all([
    searchListings(filters),
    getLendersWithCounts(filters),
    getDistrictsWithCounts(filters),
  ])
  ...
  if (filters.district) {
    activeChips.push({ label: filters.district, href: filterHref(base, searchParams, { district: null }) })
  }
    ...
    <div className="container mx-auto px-4 md:px-6 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-8">
        ...
        <aside className="hidden lg:block bg-background border border-border rounded-xl shadow-sm overflow-hidden h-fit sticky top-24">
          ...
          {/* Hidden until the ingest pipeline has resolved districts for at
              least one listing — an empty facet is a dead control. */}
          {districts.length > 0 && (
            <div className="p-4 border-b border-border">
              <h4 className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground mb-3">District</h4>
              <div className="space-y-2.5">
                {districts.map(({ district, count }) => {
                  const checked = filters.district === district
                  return (
                    <Link
                      key={district}
                      href={filterHref(base, searchParams, { district: checked ? null : district })}
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm text-muted-foreground group-hover:text-foreground transition-colors">{district}</span>
                      </div>
                      <span className="text-[11px] text-muted-foreground bg-secondary/60 px-2 py-0.5 rounded-full">{count}</span>
                    </Link>
                  )
                })}
              </div>
