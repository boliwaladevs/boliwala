...
export function AuthView({ defaultTab = "login", stats, variant = "customer" }: AuthViewProps) {
  ...
  const handleGoogleLogin = async () => {
    ...
    if (error) {
      toast({ variant: "destructive", title: "Couldn't sign in with Google", description: error.message })
    }
  }
      ...
      <div className="w-full md:w-1/2 lg:w-[55%] flex items-center justify-center p-6 sm:p-12 lg:p-20 relative">
        ...
        <div className="w-full max-w-[440px] mt-12 md:mt-0">
          ...
          <form className="flex flex-col gap-5" onSubmit={handleAuthAction}>
            ...
            <button type="button" onClick={handleGoogleLogin} className="w-full flex items-center justify-center gap-3 bg-background border border-border hover:bg-secondary/50 text-foreground font-semibold h-12 rounded-xl transition-colors text-sm">
