...
## 1. Cloudflare migration  ⬅ **1a COMPLETE — GO, 2026-08-31. `MEMORY.md` §35. Remaining: 1b–1d, blocked on the domain (D2).**
...
- **1a — Go/no-go spike. ✅ GO (2026-08-31). `MEMORY.md` §35.** First green Workers build 2026-08-30 night; live at `https://boliwala.boliwaladevs.workers.dev`; `/`, `/search`, `/login` all 200. **Resolved 2026-08-31** — those `/listing/[slug]` 500s were a missing Worker secret, not the adapter (§30.4, `MEMORY.md` §34.8 and §35); the leak test now passes 12/12 against the deployed Worker. Google login against the Worker still needs its origin added to the Google Cloud console — expected, and not a no-go. `@opennextjs/cloudflare` builds on Next 16.0.10; deploys to a `*.workers.dev` preview; `scripts/leak-test.mjs` + `scripts/access-matrix-test.mjs` + the route sweep pass against the preview; real Supabase email login + Google login work against the preview; Worker bundle < 10 MB compressed.
...
- **1a result:** the blocker was never the adapter — every `/listing/[slug]` 500 was a missing `SUPABASE_SERVICE_ROLE_KEY` secret on the Worker (`MEMORY.md` §34.8). Secret set 2026-08-31; leak test **12/12 PASS against the deployed Worker**, access matrix 49/49, 22-route sweep matching the local baseline. Google login on the Worker origin remains untestable until the origin is added in the Google Cloud console — expected, **not** a no-go.
