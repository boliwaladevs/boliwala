# MEMORY_ARCHIVE.md — Boliwala.com, §1 to §38

**Cut from `MEMORY.md` on 1 September 2026, verbatim and unedited, under the original
section numbers.** It covers 4 August to 31 August 2026: sprint completion records, the
infra direction change, the Cloudflare migration, the overnight loop briefs, and every
correction issued along the way.

> ## ⚠️ THIS IS HISTORY, NOT CURRENT STATE.
>
> Much of what is marked ❌ *not built* here **was built on 1 September 2026.** Where
> this file and `MEMORY.md` disagree, **`MEMORY.md` wins, always.**
>
> `MEMORY.md` §H is the map of what is worth opening in here and why. The short
> version: **§25** for why the architecture is what it is, **§27** for the OpenNext
> spike and the first bundle measurement, **§37.10** for the `profiles.role` correction.
